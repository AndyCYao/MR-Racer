...
public class BEUnityPluginOnLoad : MonoBehaviour
{
	...
	private static readonly string IGNORE_UNITY_VERSION_CHECK_PATH = "BridgeEngineIgnoreUnityVersion.check";
	private static readonly string REENABLE_UNITY_VERSION_CHECK_MESSAGE =
		"Unity Version check can be re-enabled by deleting " + IGNORE_UNITY_VERSION_CHECK_PATH;
#endif

	static BEUnityPluginOnLoad ()
	{
#if !UNITY_5_6_OR_NEWER
		if( System.IO.File.Exists(IGNORE_UNITY_VERSION_CHECK_PATH) == false )
		{
			// Check we are using the compatible Unity version.
			var option = EditorUtility.DisplayDialogComplex("Incompatible Unity Version", "BridgeEngine currently requires Unity 5.6 or newer to run correctly.", "Got It", null, "Got It and Never Tell Me Again" );
			if( option == 2 ) {
				System.IO.File.Create(IGNORE_UNITY_VERSION_CHECK_PATH);
				Debug.Log("<b>Bridge Engine</b>: " + IGNORE_UNITY_VERSION_CHECK_PATH + " created. Delete it to re-enable check");
				UnityEditor.EditorUtility.DisplayDialog("Skipping Unity Version check", REENABLE_UNITY_VERSION_CHECK_MESSAGE, "Ok");
			}
		}
#endif
		...
		// Works with both Metal and OpenGL ES2.
		PlayerSettings.SetUseDefaultGraphicsAPIs(BuildTarget.iOS, false);

		#if UNITY_2017_3_OR_NEWER

		// Make sure we are using Metal or OpenGL ES2;
		// If not, then use Metal by default, it's much quicker.
		var useDefaultGraphicsAPIs = PlayerSettings.GetUseDefaultGraphicsAPIs(BuildTarget.iOS);
		var firstGraphicsAPI = PlayerSettings.GetGraphicsAPIs(BuildTarget.iOS)[0];
		if(useDefaultGraphicsAPIs
			 ||
			(firstGraphicsAPI != UnityEngine.Rendering.GraphicsDeviceType.Metal
			 && firstGraphicsAPI != UnityEngine.Rendering.GraphicsDeviceType.OpenGLES2)
		) {
			Debug.Log("BEUnityPluginOnLoad: Switching to Metal API.");
		...
		if(useDefaultGraphicsAPIs
			...
			Debug.Log("BEUnityPluginOnLoad: Switching to OpenGL ES2 API.  Metal is not supported until Unity 2017.3 or newer.");

			// Prior to Unity 2017.3 Metal and GoogleVR do not work correctly together.
			PlayerSettings.SetGraphicsAPIs(BuildTarget.iOS,
				new UnityEngine.Rendering.GraphicsDeviceType[] {
