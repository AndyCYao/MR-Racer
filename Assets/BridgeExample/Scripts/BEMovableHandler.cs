...
public class BEMovableHandler : MonoBehaviour {
    [Tooltip("Highlight GameObject appears when the pointer hovers over.")]

    public GameObject hightlightObject;

    [Tooltip("Grab Location snaps to where you grabbed the object. This should be an empty child GameObject that is our grab handle.  Leave None if you don't want snapping.")]
	...
	void Start () {
        ...
        // Requires highlight object is different than this object.
        Assert.AreNotEqual(hightlightObject.gameObject, gameObject);
